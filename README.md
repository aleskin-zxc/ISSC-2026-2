...
В книге *Шумская Н. И. Определитель рудных минералов по спектральным кривым отражения. - Л.: Недра, 1985. - 231 с.* (нету в цифровом издании) классификация минералов основана на **отражении** и был создан атлас СКО(спекральные кривые отражения), которые были типизированы и по которым можно было легко сравнить результаты СКО для определяемого минерала и найти в книге (краткое изложение [1985.Шумская Н. И. Определитель рудных минералов по спектральным кривым отражения.SUMMARY- Л. Недра - 231 с.](1.definition/book/1985.Шумская.Определитель_рудных_минералов_по_спектральным_кривым_отражения.SUMMARY-Л.Недра-231с..md)).
...
3.  `formula : str` == формула, которая описывает минеральный химический состав (сама систематика формул IMA описывается в статье "[A compendium of IMA-Approved Mineral Nomenclature](article/2019.IMA_Nomenclature.FreeToUse.pdf)". Из сайта RRUFF можно скачать .csv с формулами.
4. `elements : list[str]` == список химических элементов входят в состав минерала
5. `system : Enum` == сингония минералов (кубическая, гексагональная, тригональная, тетрагональная, ромбическая, моноклинная, триклинная)
6. `anisotropy : Enum` == ('anisotropic', 'isotropic')
...
7. `moh : float(0.0:10.0)` == твердость по шкале Мооса
8. `reflectance Y : float(0.0:100.0)` == 
9. `chromaticity coordinates x : float(0,0:1,0)` == 
10. `chromaticity coordinates y : float(0,0:1,0)` == 
11. `density` == плотность минерала
